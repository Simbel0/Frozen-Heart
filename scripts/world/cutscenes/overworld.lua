return {
	incorrectway=function(cutscene)
		...
		if Game:getFlag("incorrectWayTaken", 0)==0 then 		
			if noelle then
				...
				cutscene:text("* (Hey, weird voice, that's the wrong way!)", "shy_b", "susie")
			else
	...
	incorrectFloor=function(cutscene)
		...
		if Game:getFlag("incorrectWayTaken", 0)==0 then 		
			if noelle then
				cutscene:text("* Susie..? Where are we going?", "confused_surprise_b", "noelle")
				cutscene:text("* That's what I'd like to know too.", "nervous_side", "susie")
				cutscene:text("* (Hey, weird voice, we don't need to go down!!)", "shy_b", "susie")
			else
	...
	doubts=function(cutscene)
		...
		if noelle then
			...
			cutscene:text("* What would you do if a friend of yours suddenly started being hurtful?", nil, "noelle")
			...
			cutscene:text("* Then something must have happened to them first.", "neutral_side", "susie")
			cutscene:text("* And if so,[wait:0.5] they probably need help.", "neutral", "susie")
			noelle:setSprite("head_lowered_look_right")
			cutscene:text("* ...How would slapping them help them?", nil, "noelle")
			...
			cutscene:text("* You're welcome,[wait:0.5] I guess?", "blush", "susie")
			cutscene:wait(2)
			noelle:setSprite("noelle_susie_shoulder_susiefixed")
			cutscene:text("* So...[wait:1] You're planning on keeping your hand on my shoulder forever or...?", "blush", "susie")
			susie.visible = true
	...
	together=function(cutscene)
		...
		if noelle then
			...
			cutscene:text("* Access Denied Try Another Question", "smile_side_r", "queen")
			...
			cutscene:text("* Come on.[wait:1] Who relaxes in front of a rainbow geyser?", "nervous", "susie")
			...
			cutscene:text("* You're bringing Noelle with you too?", "neutral", "ralsei")
			cutscene:look(susie, "right")
			cutscene:text("* Of course.[wait:1] two friends will be better than one,[wait:0.5] right?", "smile", "susie")
			...
			cutscene:text("* She was acting weird but everything is settled now.", "smirk", "susie")
			...
			cutscene:text("* Wha-[wait:0.5] Why would you even think that??", "shock_nervous", "susie")
			...
			cutscene:text("* Unnecessary Drama", "nice", "queen")
