...
function Mod:registerDebugOptions(d)
    ...
    d:registerOption("frozen_heart_ending", "Secret Ending", "Start the secret battle", function()
        Game:setFlag("plot", 10)
        Game:addPartyMember("kris", 1)
        Game:addPartyMember("ralsei", 3)
        Game.world:loadMap("fountain_room")
        Game:encounter("secret_battle", false)
        Kristal.DebugSystem:closeMenu()
    end, in_overworld)
    ...
    end, function() return in_battle() and Game.battle.encounter.id == "noelle_battle" end)
end
